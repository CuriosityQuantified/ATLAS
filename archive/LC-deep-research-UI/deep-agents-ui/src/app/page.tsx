...
export default function HomePage() {
  ...
  const handleNewThread = useCallback(() => {
    // Create a completely new thread, don't carry over any state
    setThreadId(null);
  ...
  const handleWorkspaceSelect = useCallback((workspaceId: string) => {
    ...
    // If this is a new workspace without a thread, ensure we start fresh
    const workspace = workspaces.find(w => w.id === workspaceId);
    if (workspace && !workspace.threadId) {
      setThreadId(null);
    }
  }, [selectWorkspace, setCurrentWorkspace, workspaces]);
