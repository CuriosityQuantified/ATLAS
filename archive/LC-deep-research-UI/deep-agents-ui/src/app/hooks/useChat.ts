...
export function useChat(
  ...
  // Restore state from backend on mount or threadId change
  useEffect(() => {
    if (!threadId || !accessToken) return;
    
    const restoreThreadState = async () => {
      try {
        const client = createClient(accessToken);
        // Fetch the current state of the thread from the backend
        const response = await client.threads.getState(threadId);
        
        if (response?.values) {
          // Check if there's an active interrupt in the thread state
          const threadState = response.values as any;
          
          // Look for interrupt state in various possible locations
          if (threadState.interrupt) {
            setCurrentInterrupt(threadState.interrupt);
          } else if (threadState.messages?.length > 0) {
            // Check the last message for interrupt state
            const lastMessage = threadState.messages[threadState.messages.length - 1];
            if (lastMessage?.interrupt || lastMessage?.additional_kwargs?.interrupt) {
              setCurrentInterrupt(lastMessage.interrupt || lastMessage.additional_kwargs.interrupt);
            }
          }
          
          // Also check for pending tool calls that might be ask_user_question
          const pendingQuestions = threadState.messages?.filter((msg: any) => 
            msg.type === "ai" && 
          ...
          if (pendingQuestions?.length > 0) {
            console.log("Found pending questions in thread state:", pendingQuestions);
          }
        }
      } catch (error) {
        console.error("Error restoring thread state:", error);
      }
    };
