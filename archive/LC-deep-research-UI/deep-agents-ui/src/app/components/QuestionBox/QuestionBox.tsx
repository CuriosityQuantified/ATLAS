...
export const QuestionBox: React.FC<QuestionBoxProps> = ({ toolCall, sendMessage, sendQuestionResponse }) => {
  ...
  const isQuestionAcknowledgment = (message: string | undefined): boolean => {
    ...
    const questionLower = toolCall.args?.question?.toLowerCase() || "";
    ...
    return (
      ...
      // Check if it's exactly the question wrapped in a standard format
      ...
      lowerMessage.includes("please answer") ||
      // Check if the result is just the question being echoed back
      (questionLower && lowerMessage === questionLower)
    );
  };
  
  // Create a unique key for this question box based on tool call ID
  const storageKey = `questionBox_${toolCall.id}`;
  ...
  const getPersistedState = () => {
    try {
      ...
      console.error("Error reading persisted question state:", error);
    }
    return null;
  };
  
  // Derive initial state from toolCall or localStorage
  const persistedState = getPersistedState();
  const hasRealAnswer = toolCall.status === "completed" && 
                       toolCall.result && 
                       !isQuestionAcknowledgment(toolCall.result);
  
  // Use persisted answer if available, otherwise check toolCall
  const existingAnswer = persistedState?.submittedAnswer || 
                        (hasRealAnswer ? (toolCall.result || "") : "");
  const initialIsAnswered = persistedState?.isAnswered || hasRealAnswer;
  ...
  useEffect(() => {
    if (isAnswered && submittedAnswer) {
      try {
        ...
        console.error("Error persisting question state:", error);
      }
    }
  }, [isAnswered, submittedAnswer, storageKey]);
