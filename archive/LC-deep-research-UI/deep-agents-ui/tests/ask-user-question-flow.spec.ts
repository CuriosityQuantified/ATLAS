...
test.describe('Ask User Question Flow', () => {
  ...
  test('Complete question-answer flow with persistence', async ({ page }) => {
    ...
    // Look for the question box by finding the unique textarea placeholder
    // The QuestionBox component has a textarea with this specific placeholder
    const questionTextarea = page.locator('textarea[placeholder*="Type your answer (Enter to send"]');
    await expect(questionTextarea).toBeVisible({ timeout: 20000 });
    
    // Get the parent container which is the question box
    const questionBoxSelector = 'div[class*="questionBox"]';
    ...
    // We already found the textarea, so just use it directly
    const answerInput = questionTextarea;
    ...
    // Click the send button - look for button with "Send answer" text
    const sendButton = page.locator('button:has-text("Send answer")');
    await expect(sendButton).toBeVisible();
    await expect(sendButton).toBeEnabled({ timeout: 5000 }); // Wait for it to be enabled
  ...
  test('Multiple questions in same session', async ({ page }) => {
    ...
    const questionTextarea = page.locator('textarea[placeholder*="Type your answer (Enter to send"]');
    await expect(questionTextarea).toBeVisible({ timeout: 20000 });
    await questionTextarea.fill('Blue');
    const sendButton = page.locator('button:has-text("Send answer")').first();
    ...
    await page.waitForTimeout(2000); // Give time for new question to appear
    const secondAnswerInput = page.locator('textarea[placeholder*="Type your answer (Enter to send"]').last();
    await expect(secondAnswerInput).toBeVisible({ timeout: 10000 });
    await secondAnswerInput.fill('Pizza');
    const secondSendButton = page.locator('button:has-text("Send answer")').last();
